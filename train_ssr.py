...
from models.baseline.ssr import SSR, spatial_D, weights_init_kaiming
    ...
    while logger.itr < config['max_itr']:
        ...
        for ind_f, f in enumerate(file_list):
            ...
            for x, x_low in train_dataloader:
                ...
                output_fake = Ds(fake_data)
                output_real = Ds(x)
                label_real_s = torch.ones(output_real[-1].size()).cuda()
                # adversarial loss
                L_adv = criterion(output_fake[-1],label_real_s)
                # perceptual loss
                L_p = criterion(output_fake[0],output_real[0])+criterion(output_fake[1],output_real[1])+criterion(output_fake[2],output_real[2])+criterion(output_fake[3],output_real[3])
                # content loss
                L_c = criterion(fake_data, x)
