...
class SphereTemplate(Template):
    ...
    def get_random_points(self, shape, device="cpu"):
        """
        Get random points on a Sphere
        Return Tensor of Size [x, 3, x ... x]
        """
        assert shape[1] == 3, "shape should have 3 in dim 1"
        # rand_grid = torch.FloatTensor(shape).to(device).float()
        rand_grid = torch.rand(shape).to(device).float()
    ...
    def get_regular_points(self, npoints=None, device="cpu"):
        """
...
class SquareTemplate(Template):
    ...
    def get_regular_points(self, npoints=2500, device="cpu"):
        """
        Get regular points on a Square
        Return Tensor of Size [x, 3]
        """
        # print(npoints, device)
        if not self.npoints == npoints:
            self.npoints = npoints
