...
# from models.modules.flows.spline.cubic import cubic_spline
# from models.modules.flows.spline.linear_rational import rational_linear_spline
# from models.modules.flows.spline.quadratic_rational import rational_quadratic_spline


from models.modules.flows.linear import IdentityLayer

logabs = lambda x: torch.log(torch.abs(x) + 10e-9)
...
class AffineCouplingLayer(nn.Module):
    ...
    def forward(self, x: Tensor, c: Tensor=None, **kwargs):
        ...
        if self.coupling == 'affine':
            ...
            # log_det_J = torch.sum(scale.flatten(start_dim=1), 1) # sum(s) 
            
            scale = torch.sigmoid(scale + 2.)
            h2 = h2 * scale + bias
            log_det_J = torch.sum(torch.log(scale).flatten(start_dim=1), 1) # sum(log|s|)
