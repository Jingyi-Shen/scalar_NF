...
from models.modules.flows.spline import searchsorted, cbrt # cubic root
...
def cubic_spline(inputs,
    ...
    eps = DEFAULT_EPS, quadratic_threshold = DEFAULT_QUADRATIC_THRESHOLD):
    ...
    outputs[inside_interval_mask], log_det_J[inside_interval_mask] = _cubic_spline(
        inputs[inside_interval_mask],
        unnormalized_width[inside_interval_mask],
        unnormalized_height[inside_interval_mask],
        unnorm_derivative_left[inside_interval_mask, :],
        unnorm_derivative_right[inside_interval_mask, :],
        inverse, min_bin_width, min_bin_height,
        num_bins, -tail_bound, tail_bound, -tail_bound, tail_bound, eps, quadratic_threshold)

    return outputs, log_det_J

def _cubic_spline(inputs,
    unnormalized_widths, unnormalized_heights, unnorm_derivatives_left, unnorm_derivatives_right,
    inverse, min_bin_width, min_bin_height,
    num_bins, left, right, bottom, top, eps, quadratic_threshold):
    """
    References:
