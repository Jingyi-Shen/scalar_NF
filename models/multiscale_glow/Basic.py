...
from models.multiscale_glow.ActNorms import ActNorm2d, ActNorm3d
...
# # added
# class Conv3dZeros(nn.Module):
#     def __init__(self, in_channel: int, out_channel: int, padding="same", log_scale=3.0):
#         super(Conv3dZeros, self).__init__()
#         self.logscale_factor = log_scale
#         self.bias = nn.Parameter(torch.zeros(1, out_channel, 1, 1, 1))
#         self.logs = nn.Parameter(torch.zeros(1, out_channel, 1, 1, 1))
#         self.conv3d = nn.Conv3d(in_channel, out_channel, kernel_size=1, bias=False)
...
class GaussianDiag:
    ...
    @staticmethod
    def sample(mean, logs, eps_std=None, eps=None):
