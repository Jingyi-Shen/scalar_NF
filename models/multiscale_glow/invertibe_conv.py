...
if __name__ == "__main__":
    size = (16,8,32,32)
    invconv = InvertibleConvolution(8,device)
    opt     = torch.optim.Adam(invconv.parameters(), lr=0.01)
    for i in range(50):
        opt.zero_grad()
        ...
        print(" loss=%0.3f | mu=%0.3f | std=%0.3f | rloss=%0.3f | logdet=%0.3f" # \r
              %(loss.item(),mu,std,loss_rev, logdet.item()))
